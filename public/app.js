...
function loadActionsForEntity(domain) {
    ...
    if (domain === 'climate') {
        ...
        restoreOption.value = 'restore_state';
        restoreOption.textContent = 'Restore Last State';
        actionSelect.appendChild(restoreOption);
    }
...
function handleActionChange() {
    ...
    // Handle restore_state action for climate entities
    if (action === 'restore_state' && domain === 'climate') {
        const state = selectedEntity.state;
        const attrs = selectedEntity.attributes || {};
        const previewHtml = `
            <div class="restore-state-preview">
                <div class="preview-title">State to restore:</div>
                <div class="preview-item">🌡️ HVAC Mode: <strong>${state || 'N/A'}</strong></div>
                ${attrs.temperature != null ? `<div class="preview-item">🎯 Target Temperature: <strong>${attrs.temperature}°</strong></div>` : ''}
                ${attrs.fan_mode ? `<div class="preview-item">💨 Fan Mode: <strong>${attrs.fan_mode}</strong></div>` : ''}
            </div>
        `;
        parameterInputs.innerHTML = previewHtml;
...
async function scheduleAction() {
    ...
    if (scheduleType === 'relative') {
        const timeValue = document.getElementById('relativeTime').value;
        const timeUnit = document.getElementById('relativeUnit').value;
        if (!timeValue || timeValue < 1) {
            const unitLabel = timeUnit === 'minutes' ? 'minute' : 'second';
            alert(`Please enter a valid time value (minimum 1 ${unitLabel})`);
            return;
        }
        // Convert to seconds for the server
        scheduleValue = timeUnit === 'minutes' ? String(parseInt(timeValue) * 60) : String(parseInt(timeValue));
    ...
    // For 'immediate', no scheduleValue is needed
    
    // Collect service data from parameters
    const serviceData = {};
    const domain = selectedEntity.entity_id.split('.')[0];
    
    // Handle restore_state: capture the entity's current state
    let actualService = action;
    if (action === 'restore_state' && domain === 'climate') {
        actualService = 'restore_state';
        const state = selectedEntity.state;
        const attrs = selectedEntity.attributes || {};
        ...
        if (attrs.temperature != null) {
            serviceData.temperature = attrs.temperature;
        }
        if (attrs.fan_mode) {
            serviceData.fan_mode = attrs.fan_mode;
        }
    } else {
        const parameters = getParametersForAction(domain, action);
        parameters.forEach(param => {
