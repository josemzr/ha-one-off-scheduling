...
You can run the application (Web UI + MCP server) using Docker Compose, separate from the HA add-on.
...
1. **Create a `.env` file** with your configuration:
   ...
   MCP_AUTH_TOKEN=your_secret_mcp_token
   ```
...
3. **Verify**:
   ...
   - MCP endpoint: `http://localhost:3000/mcp` (requires `Authorization: Bearer <MCP_AUTH_TOKEN>` header)
