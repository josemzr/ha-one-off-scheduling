...
The add-on uses Home Assistant's ingress feature for the web UI, which means:
- No additional configuration is needed for the web interface
...
The add-on includes an integrated MCP (Model Context Protocol) server that allows AI assistants (VS Code Copilot, Claude Desktop, etc.) to programmatically schedule actions.

The MCP endpoint is exposed on port **3000** (bypassing ingress) so external clients can connect directly.

**Configuration options:**

| Option | Description |
|--------|-------------|
| `mcp_auth_token` | Bearer token to protect the MCP endpoint. Leave empty to disable MCP authentication (not recommended). |
