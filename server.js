...
const { mountRestApi } = require('./rest-api');
const { initDb, saveJob, deleteJob, loadJobs, getMaxJobId, closeDb } = require('./db');
...
// Initialize database and restore persisted jobs
initDb();
jobCounter = getMaxJobId();

/**
 * Restore jobs that were persisted before the last shutdown.
 * Jobs whose schedule date is still in the future are re-scheduled;
 * expired jobs are removed from the database.
 */
function restoreJobs() {
  ...
  for (const saved of persisted) {
    const scheduleDate = new Date(saved.scheduleDate);

    if (scheduleDate <= new Date()) {
      // Schedule time has passed — clean up
      deleteJob(saved.id);
      expired++;
      continue;
    }
    ...
    const job = schedule.scheduleJob(scheduleDate, async () => {
      try {
        await executeAction(saved.entityId, saved.service, saved.serviceData || {});
        console.log(`Executed restored action: ${saved.service} on ${saved.entityId}`);
      } catch (error) {
        console.error(`Failed to execute restored action: ${error.message}`);
      } finally {
        scheduledJobs.delete(saved.id);
        deleteJob(saved.id);
      }
    });

    scheduledJobs.set(saved.id, {
      id: saved.id,
      entityId: saved.entityId,
      service: saved.service,
      serviceData: saved.serviceData,
  ...
  if (restored > 0 || expired > 0) {
    console.log(`Restored ${restored} scheduled job(s), removed ${expired} expired job(s)`);
  }
}
...
async function scheduleAction({ entityId, service, serviceData, scheduleType, scheduleValue }) {
  ...
  scheduledJobs.set(jobId, { id: jobId, entityId, service, serviceData, scheduleDate: isoDate, job });
  saveJob({ id: jobId, entityId, service, serviceData, scheduleDate: isoDate });

  return { success: true, jobId, scheduleDate: isoDate };
}
...
// Mount authenticated REST API at /api/v1
mountRestApi(app, { callHomeAssistant, scheduleAction, getScheduledJobs, cancelScheduledJob });
...
app.listen(PORT, () => {
  ...
  // Restore persisted jobs after the server is listening
  restoreJobs();
});
